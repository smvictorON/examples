...
let tmp1 = [
	...
	},
	{
		...
		children: [
			{
				fileName: 'file3.txt',
				filePath: 'tmp1/logs/file3.txt',
				contents: 'FASTEST INTERNET SPEED\nFASTEST INTERNET SPEED AGAIN\nFASTEST AGAIN AGAIN',
				isDirectory: false
			},
			{
				fileName: 'file4.txt',
				filePath: 'tmp1/logs/file5.txt',
				contents: 'F',
				isDirectory: false
			}
		]
...
function createFiles() {
	result.forEach(file => {
		fs.writeFile(file.filePath, '', (err) => {
			if (err) {
				console.error('Error creating file:', file.filePath);
			} else {
				console.log('File created successfully:', file.filePath);
			}
		});
	});
}
